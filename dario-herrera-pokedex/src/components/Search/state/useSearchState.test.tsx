...
describe("useSearchState", () => {
  ...
  it("should return an state when useSearchState is called", () => {
    const { result } = renderHook(() => useSearchState());

    expect(result.current.isNotFoundPokemon).toEqual(false);
    expect(result.current.isRenderMessage).toEqual(false);
    expect(result.current.handleOnInputChance).toBeTruthy();
  });
});
